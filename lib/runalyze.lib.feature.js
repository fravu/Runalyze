...
Runalyze.Feature = (function($, Parent){
	...
	function initAjaxLinks() {
		$('a.ajax').unbind('click').click(function(e){
			...
			if (href != '#') {
				...
				} else if (target == 'data-browser-inner') {
					// Keep #refreshDataBrowser in sync with the currently
					// displayed period, otherwise saving an activity (which
					// reloads the page via that stale link) jumps back to
					// whatever period was shown on the initial page load.
					$('#refreshDataBrowser').attr('href', href);
				}
	...
	function initCalendar() {
		...
		$('#calendar-submit').unbind('click').bind('click', function(){
			...
			$('#refreshDataBrowser').attr('href', href);
			$("#data-browser-inner").loadDiv(href);
		});
